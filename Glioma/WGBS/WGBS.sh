...
## enhancers intersect with DM CpG +/- 20bp - MEME
export PATH=$PATH:/home/lli/bin/meme-5.1.0/bin:/home/lli/bin/meme-5.1.0/libexec/meme-5.1.0
dirMotif=/home/lli/bin/meme-5.1.0/db/motif_databases/
BEDTOOLS=/gsc/software/linux-x86_64-centos5/bedtools/bedtools-2.25.0/bin/
hg19=/projects/alignment_references/9606/hg19a/genome/bwa_32/hg19a.fa
...
for file in $dirIn/DM.IDH*_NPC.bed; do
    name=$(basename $file | cut -d'.' -f2); echo $name
    less $file | awk '$4==1 {print $1"\t"$2-20"\t"$3+20}' | $BEDTOOLS/mergeBed -i stdin | $BEDTOOLS/intersectBed -a stdin -b $enhancer -u -f 1 | $BEDTOOLS/closestBed -a stdin -b <(less $tss | sed 's/chr//g') -d | awk '$8<20000 {print $1"\t"$2"\t"$3"\t"$1":"$2"-"$3"_"$7}' | $BEDTOOLS/bedtools getfasta -fi $hg19 -bed stdin -name -fo $dirIn/DM.$name.hyper.enhancer.fa
    ...
    less $file | awk '$4==-1 {print $1"\t"$2-20"\t"$3+20}' | $BEDTOOLS/mergeBed -i stdin | $BEDTOOLS/intersectBed -a stdin -b $enhancer -u -f 1 | $BEDTOOLS/closestBed -a stdin -b <(less $tss | sed 's/chr//g') -d | awk '$8<20000 {print $1"\t"$2"\t"$3"\t"$1":"$2"-"$3"_"$7}' | $BEDTOOLS/bedtools getfasta -fi $hg19 -bed stdin -name -fo $dirIn/DM.$name.hypo.enhancer.fa
...
less $dirIn/DM.IDHmut_NPC.bed | awk '$4==1 {print $1"\t"$2-20"\t"$3+20}' | $BEDTOOLS/mergeBed -i stdin | $BEDTOOLS/intersectBed -a stdin -b $enhancer -u -f 1 | $BEDTOOLS/intersectBed -a stdin -b <(less $dirIn/DM.IDHwt_NPC.bed | awk '$4==1 {print $0}') -v | $BEDTOOLS/closestBed -a stdin -b <(less $tss | sed 's/chr//g') -d | sort -k7,7 | join - <(less $up_NPC | sort -k1,1) -1 7 -2 1 | awk -F" " '$8<20000 {print $2"\t"$3"\t"$4"\t"$2":"$3"-"$4"_"$1}' | $BEDTOOLS/bedtools getfasta -fi $hg19 -bed stdin -name -fo $dirIn/DM.IDHmut.hyper.enhancer.UP_NPC.fa
...
less $dirIn/DM.IDHmut_NPC.bed | awk '$4==1 {print $1"\t"$2-20"\t"$3+20}' | $BEDTOOLS/mergeBed -i stdin | $BEDTOOLS/intersectBed -a stdin -b $enhancer -u -f 1 | $BEDTOOLS/intersectBed -a stdin -b <(less $dirIn/DM.IDHwt_NPC.bed | awk '$4==1 {print $0}') -v | $BEDTOOLS/closestBed -a stdin -b <(less $tss | sed 's/chr//g') -d | sort -k7,7 | join - <(less $up_wt | sort -k1,1) -1 7 -2 1 | awk -F" " '$8<20000 {print $2"\t"$3"\t"$4"\t"$2":"$3"-"$4"_"$1}' | $BEDTOOLS/bedtools getfasta -fi $hg19 -bed stdin -name -fo $dirIn/DM.IDHmut.hyper.enhancer.UP_wt.fa
