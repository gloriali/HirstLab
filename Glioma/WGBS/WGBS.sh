...
## enhancers intersect with DMRs - AME
...
## enhancers intersect with DM CpG +/- 50bp - CentriMo
export PATH=$PATH:/home/lli/bin/meme-5.1.0/bin:/home/lli/bin/meme-5.1.0/libexec/meme-5.1.0
motif=/home/lli/bin/meme-5.1.0/db/motif_databases/METHYLCYTOSINE/yin2017.name.meme 
BEDTOOLS=/gsc/software/linux-x86_64-centos5/bedtools/bedtools-2.25.0/bin/
hg19=/projects/alignment_references/9606/hg19a/genome/bwa_32/hg19a.fa
tss=/home/lli/hg19/hg19v69_genes.TSS.pc.bed
enhancer=/projects/epigenomics3/epigenomics3_results/users/lli/glioma/ChIPseq/FindER2/H3K27ac.IDHmut.bed
...
dirIn=/projects/epigenomics3/epigenomics3_results/users/lli/glioma/WGBS/limma/
mkdir -p $dirIn/centrimo/
for file in $dirIn/DM.IDH*_NPC.bed; do
    name=$(basename $file | cut -d'.' -f2); echo $name
    less $file | awk '$4==1 {print $1"\t"$2-50"\t"$3+50}' | $BEDTOOLS/intersectBed -a stdin -b $enhancer -u -f 1 | $BEDTOOLS/closestBed -a stdin -b <(less $tss | sed 's/chr//g') -d | awk '{print $1"\t"$2"\t"$3"\t"$1":"$2"-"$3"_"$7}' | $BEDTOOLS/bedtools getfasta -fi $hg19 -bed stdin -name -fo $dirIn/DM.$name.hyper.enhancer.fa
    centrimo --oc $dirIn/centrimo/$name.hyper/ $dirIn/DM.$name.hyper.enhancer.fa $motif
    less $file | awk '$4==-1 {print $1"\t"$2-50"\t"$3+50}' | $BEDTOOLS/intersectBed -a stdin -b $enhancer -u -f 1 | $BEDTOOLS/closestBed -a stdin -b <(less $tss | sed 's/chr//g') -d | awk '{print $1"\t"$2"\t"$3"\t"$1":"$2"-"$3"_"$7}' | $BEDTOOLS/bedtools getfasta -fi $hg19 -bed stdin -name -fo $dirIn/DM.$name.hypo.enhancer.fa
    centrimo --oc $dirIn/centrimo/$name.hypo/ $dirIn/DM.$name.hypo.enhancer.fa $motif
done
less $dirIn/DM.IDHmut_NPC.bed | awk '$4==1 {print $1"\t"$2-50"\t"$3+50}' | $BEDTOOLS/intersectBed -a stdin -b $enhancer -u -f 1 | $BEDTOOLS/intersectBed -a stdin -b <(less $dirIn/DM.IDHwt_NPC.bed | awk '$4==1 {print $0}') -v | $BEDTOOLS/closestBed -a stdin -b <(less $tss | sed 's/chr//g') -d | sort -k7,7 | join - <(less $up_NPC | sort -k1,1) -1 7 -2 1 | awk -F" " '$8<20000 {print $2"\t"$3"\t"$4"\t"$2":"$3"-"$4"_"$1}' | $BEDTOOLS/bedtools getfasta -fi $hg19 -bed stdin -name -fo $dirIn/DM.IDHmut.hyper.enhancer.UP_NPC.fa
centrimo --oc $dirIn/centrimo/IDHmut.hyper.enhancer.UP_NPC/ $dirIn/DM.IDHmut.hyper.enhancer.UP_NPC.fa $motif
less $dirIn/DM.IDHmut_NPC.bed | awk '$4==1 {print $1"\t"$2-50"\t"$3+50}' | $BEDTOOLS/intersectBed -a stdin -b $enhancer -u -f 1 | $BEDTOOLS/intersectBed -a stdin -b <(less $dirIn/DM.IDHwt_NPC.bed | awk '$4==1 {print $0}') -v | $BEDTOOLS/closestBed -a stdin -b <(less $tss | sed 's/chr//g') -d | sort -k7,7 | join - <(less $up_wt | sort -k1,1) -1 7 -2 1 | awk -F" " '$8<20000 {print $2"\t"$3"\t"$4"\t"$2":"$3"-"$4"_"$1}' | $BEDTOOLS/bedtools getfasta -fi $hg19 -bed stdin -name -fo $dirIn/DM.IDHmut.hyper.enhancer.UP_wt.fa
centrimo --oc $dirIn/centrimo/IDHmut.hyper.enhancer.UP_wt/ $dirIn/DM.IDHmut.hyper.enhancer.UP_wt.fa $motif 
