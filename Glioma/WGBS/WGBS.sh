...
for dm in DM.*.bed; do
    ...
    echo $samples $s1 $s2
    awk 'NR==FNR {id=$1":"$2; if(($4+$5 >= "'$cov'"+0)&&($4+$5 <= 5000)){h[id]=$6}; next} {id=$1":"$2; if((id in h)&&($4+$5 >= "'$cov'"+0)&&($4+$5 <= 5000)){print "chr"$1"\t"$2"\t"$3"\t"id"\t"$6"\t"h[id]}}' $(ls $dir5mC/$s2.*5mC.CpG.*combine.5mC.CpG) $(ls $dir5mC/$s1.*5mC.CpG.*combine.5mC.CpG) > $dirIn/$samples.join
