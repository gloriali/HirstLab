...
## Unique enriched regions: IDHmut vs IDHwt 
...
(ER_unique_wt_summary_figure)
```

## Unique enriched regions: glioma vs NPC
* Pairwise comparisons between glioma samples (S1) and NPC GE04 (S2).   
* Challenge: S1 and S2 often have quite different sequencing depth, thus direct comparisons between ER are likely biased.     
* Input: ER in S1, wig in S1, ER in S2, wig in S2.   
* Method: 
...
homer_unique_K27ac_NPC_tf_RPKM_figure$data <- homer_unique_K27ac_NPC_tf_RPKM_figure$data %>% filter(Motif == "Sox3")
...
homer_unique_K4me1_NPC_tf_figure$data <- homer_unique_K4me1_NPC_tf_figure$data %>% filter(Percent_with_motif >= 20)
...
homer_unique_K4me1_IDHmut_tf_figure$data <- homer_unique_K4me1_IDHmut_tf_figure$data %>% filter(Percent_with_motif >= 20)
