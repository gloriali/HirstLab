...
less A04_MN1ND13_DP.bed.CpG.txt.gz.combine.5mC.CpG | awk '{print $1":"$2"-"$3"\t"$6}' | sort -k1,1 > DP.tmp
less A05_MN1ND13_DN.bed.CpG.txt.gz.combine.5mC.CpG | awk '{print $1":"$2"-"$3"\t"$6}' | sort -k1,1 > DN.tmp
join DP.tmp DN.tmp | sed 's/ /\t/g' | awk 'BEGIN{for(i=1;i<=21;i++){s[i]=0}} {d=int(($2-$3)*10+11); s[d]++} END{for(i=1;i<=21;i++){print (i-11)/10"\t"s[i]}}' > DP_DN.5mC.delta.summary
rm DP.tmp DN.tmp
