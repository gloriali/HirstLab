...
## QC for SOP
setwd("/projects/epigenomics3/epigenomics3_results/users/lli/SOP_QC/")
QC_summary <- read.delim("summary.xls", as.is = T, row.names = 2, head = F) %>% select(-V1) %>% t() %>% as.data.frame() 
colnames(QC_summary) <- gsub("without_Dups_and_Q_>=_10", "After_Filter", colnames(QC_summary))
QC_summary <- QC_summary %>% mutate(Library = gsub("meDIP-HL60-B05_S27", "IP-5mC-3X.2_S5", Library), Sample = gsub(".*-", "", gsub("_S.*", "", Library)), Type = gsub("IP-", "", gsub("-\\dX.*", "", Library)))
...
(QC_summary_reads_figure <- ggplot(QC_summary_reads, aes(Sample, as.numeric(value)/10^6, fill = Sample)) + 
		...
		facet_grid(variable ~ Type, scales = "free_x", space = "free_x") + 
		guides(fill = "none") + 
		xlab("") + 
		ylab("No. of million reads") +
		theme_bw() + 
		theme(axis.text.x = element_text(angle = 90)))
ggsave(QC_summary_reads_figure, file = "SOP_QC_summary_reads_figure.pdf", height = 8, width = 4)
...
(QC_summary_percent_figure <- ggplot(QC_summary_percent, aes(Sample, as.numeric(value), fill = Sample)) + 
		geom_bar(stat = "identity") + 
		facet_grid(variable ~ Type, scales = "free", space = "free_x") + 
		guides(fill = "none") + 
		xlab("") + 
		ylab("Percent of total number of reads") +
		theme_bw() + 
		theme(axis.text.x = element_text(angle = 90)))
ggsave(QC_summary_percent_figure, file = "SOP_QC_summary_percent_figure.pdf", height = 10, width = 4)
...
## spike-in coverage
spikein_coverage <- read.delim("/projects/epigenomics3/bams/hg19/UBC_meDIPQC_01NOV2017/spike_in.coverage", as.is = T) %>% mutate(Library = gsub("meDIP-HL60-B05_S27", "IP-5mC-3X.2_S5", Library), Sample = gsub(".*-", "", gsub("_S.*", "", Library)), Type = gsub("IP-", "", gsub("-\\dX.*", "", Library)), Genome = revalue(Genome, c("NC_001416.1" = "Lambda", "NC_001604.1" = "T7", "NC_003287.2" = "M13")))
(spikein_coverage_5mC_figure <- ggplot(spikein_coverage %>% filter(Type == "5mC"), aes(Sample, Coverage, fill = Sample)) + 
		...
		theme_bw())
ggsave(spikein_coverage_5mC_figure, file = "SOP_spikein_coverage_5mC_figure.pdf", height = 8, width = 3)
