...
```{r proximal_DE, results='asis', fig.height=4}
DMR_proximal_GW_summary <- xtable(DMR_proximal_GW_summary)
align(DMR_proximal_GW_summary) <- "cccccc"
print(DMR_proximal_GW_summary, type = "html", include.rownames = T)
...
kable(DMR_DE_Cortex02_Cortex04_hyper$DMR_gene[intersect(DMR_DE_Cortex02_Cortex04_hyper$DMR_gene$id, DMR_DE_GE02_GE04_hyper$DMR_gene$id), c("id", "name", "description")], row.names = F, align = c("c", "r", "r"))
```

### Proximal GE 13-week UMRs with DE genes

```{r proximal_13_GE_DE, results='asis'}
kable(DMR_DE_GE02_GE04_hyper$DMR_gene_DE[!duplicated(DMR_DE_GE02_GE04_hyper$DMR_gene_DE$id), c("id", "name", "description", "DM", "DE")], row.names = F, align = c("c", "r", "r", "c", "c"))
```

### Proximal 17-week UMRs with DE genes in both Cortex and GE

```{r proximal_17_DE, results='asis'}
kable(DMR_DE_GE02_GE04_hypo$DMR_gene_DE[intersect(DMR_DE_Cortex02_Cortex04_hypo$DMR_gene_DE$id, DMR_DE_GE02_GE04_hypo$DMR_gene_DE$id), c("id", "name", "description", "DM", "DE")], row.names = F, align = c("c", "r", "r", "c", "c"))
```
...
(DMR_GW_TF_figure)
kable(DMR_GW_TF[DMR_GW_TF$Ratio_Cortex>25 & DMR_GW_TF$Ratio_GE>25, ], row.names = F, align = c("r", "c", "c", "c", "c", "c", "c"))
```
...
## DE genes summary   
  ...
  + On average, there are __`r as.integer(mean(GW_DE_summary$DE))`__ genes differentially expressed between 13- and 17-week individuals, __`r as.integer(mean(GW_DE_summary$DN))`__ up-regulated in 13-week individuals, and __`r as.integer(mean(GW_DE_summary$UP))`__ up-regulated in 17-week individuals, __`r round(mean(GW_DE_summary$UP) / mean(GW_DE_summary$DN, 2))`__-times as many.   
  + Within each cell type, majority of DE genes are shared among different comparisons (hypergeometric p-value = 0). There are __`r nrow(GW_17_13_UP_duplicated_cortex)`__ genes up-regulated in 17-week in cortex shared by at least two comparisons, __`r nrow(GW_17_13_DN_duplicated_cortex)`__ up-regulated in 13-week in cortex, __`r nrow(GW_17_13_UP_duplicated_GE)`__ up-regulated in 17-week in GE, and __`r nrow(GW_17_13_DN_duplicated_GE)`__ up-regulated in 13-week in GE.   
  + The overlap between cell types is highly significant as well (hypergeometric p-value = 0), with __`r nrow(GW_17_13_UP_duplicated)`__ genes up-regulated in 17-week in both cortex and GE supported by at least two comparisons, and __`r nrow(GW_17_13_DN_duplicated)`__ genes up-regulated in 13-week.    

```{r DE, results='asis'}
GW_DE_summary <- xtable(GW_DE_summary)
align(GW_DE_summary) <- "cccc"
...
grid.arrange(gTree(children = venn_GW_17_13_UP), gTree(children = venn_GW_17_13_DN), nrow = 1)
```
...
## DAVID enrichment for DE genes  
  ...
  * In cortex:     
```{r DAVID_DE_cortex, fig.height=3}
(enrich_GW_17_13_UP_cortex)
(enrich_GW_17_13_DN_cortex)
```

  * In GE:    
```{r DAVID_DE_GE_UP, fig.height=8}
(enrich_GW_17_13_UP_GE)
```
```{r DAVID_DE_GE_DN, fig.height=5}
...
align(GW_isoform_summary) <- "ccccccc"
print(GW_isoform_summary, type = "html", include.rownames = T)
