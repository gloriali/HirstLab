...
* MeDIP UMR frequency (top panel, bp/Mb), No. of unique enhancers, No. of differential expressed genes (middle panel), and No. of isoform genes (bottom panel) between MZ twins (left panel), and between two neurosphere cell types (right panel). HuFNSC01-specific/cortex-specific UMRs and genes up-regulated in HuFNSC01/cortex are shown in red, and HuFNSC02-specific/GE-specific UMRs and genes up-regulated in HuFNSC02/GE are shown in blue.        
* _Note the opposite direction of UMRs and unique enhancers, why?_      

```{r MZ_scale, fig.height = 10, fig.width = 12}
MZ_scale <- data.frame(Sample = rep(c(DMR_MZ_summary$Sample, DMR_NPCs_summary$Sample), 7), 
                        Category = rep(c(rep("Individual specific", nrow(DMR_MZ_summary)), rep("Cell type specific", nrow(DMR_NPCs_summary))), 7), 
                        DM_DE = factor(c(rep(rep(c("Sample1 specific/UP", "Sample2 specific/UP"), each = nrow(DMR_MZ_summary) + nrow(DMR_NPCs_summary)), 3), rep("Isoforms", nrow(DMR_MZ_summary) + nrow(DMR_NPCs_summary))), levels = c("Sample1 specific/UP", "Sample2 specific/UP", "Isoforms")), 
                        Assay = factor(c(rep(c("UMRs", "Unique enhancers", "DE genes"), each = (nrow(DMR_MZ_summary) + nrow(DMR_NPCs_summary))*2), rep("Isoforms", (nrow(DMR_MZ_summary) + nrow(DMR_NPCs_summary)))), levels = c("UMRs", "Unique enhancers", "DE genes", "Isoforms")), 
                        value = c(-(filter(DMR_freq_MZ_figure$data, DM == -1) %>% group_by(cell) %>% summarize(m = mean(freq)))$m, -(filter(DMR_freq_NPCs_figure$data, DM == -1) %>% group_by(donor) %>% summarize(m = mean(freq)))$m, -(filter(DMR_freq_MZ_figure$data, DM == 1) %>% group_by(cell) %>% summarize(m = mean(freq)))$m, -(filter(DMR_freq_NPCs_figure$data, DM == 1) %>% group_by(donor) %>% summarize(m = mean(freq)))$m, unique_enhancers_summary[1:5, "Sample1_unique"], -unique_enhancers_summary[1:5, "Sample2_unique"], individual_DE_summary$UP[1:3], cortex_GE_DE_summary$UP[1:2], -individual_DE_summary$DN[1:3], -cortex_GE_DE_summary$DN[1:2], individual_summary[1:3, "isoform_genes"], cortex_GE_summary[1:2, "isoform_genes"]))
...
grid.text("UMR frequency", x = unit(0.015, "npc"), y = unit(0.85, "npc"), rot = 90, gp=gpar(fontsize=15))
...
pdf("Figure1c.pdf", height = 10, width = 12)
grid.newpage()
grid.draw(MZ_scale_figure_gt)
grid.text("UMR frequency", x = unit(0.015, "npc"), y = unit(0.85, "npc"), rot = 90, gp=gpar(fontsize=15))
grid.text("No. of enhancers", x = unit(0.015, "npc"), y = unit(0.62, "npc"), rot = 90, gp=gpar(fontsize=15))
...
## Figure 2: NPCs Cortex and GE
### Figure 2a: Scale of UMRs and DE genes between NPCs 

* WGBS UMR frequency (top panel, bp/Mb), fold enrichment of UMRs in enhancers, No. of differential expressed genes (middle panel), and No. of isoform genes (bottom panel) between NPCs cortex and GE in GW13 (left panel), and GW17 (right panel). cortex-specific UMRs and genes up-regulated in cortex are shown in red, and GE-specific UMRs and genes up-regulated in GE are shown in blue.        
...
NPCs_scale <- filter(NPCs_scale, Donor %in% c("HuFNSC02", "HuFNSC04"))
...
grid.text("UMR frequency", x = unit(0.015, "npc"), y = unit(0.85, "npc"), rot = 90, gp=gpar(fontsize=15))
grid.text("Enrichment", x = unit(0.015, "npc"), y = unit(0.6, "npc"), rot = 90, gp=gpar(fontsize=15))
grid.text("No. of genes", x = unit(0.015, "npc"), y = unit(0.3, "npc"), rot = 90, gp=gpar(fontsize=15))
...
* FDR for GREAT GO biological processes terms enriched (region-based binomial and hypergeometric FDR < 0.05) in NPCs cortex UMRs (red), and GE UMRs (blue) on -log10 scale.     
...
* UCSC genome browser screen shot of cortex UMR activated genes, top panel: Glial Fibrillary Acidic Protein (GFAP), bottom panel: Nuclear Factor I/X (CCAAT-Binding Transcription Factor, NFIX).     
...
* WGBS UMR frequency (top panel, bp/Mb), fold enrichment of UMRs in enhancers, No. of differential expressed genes (middle panel), and No. of isoform genes (bottom panel) between GW13 and GW17 in NPCs cortex (left panel), and GE (right panel). GW13-specific UMRs and genes up-regulated in GW13 are shown in red, and GW17-specific UMRs and genes up-regulated in GW17 are shown in blue.        
...
grid.text("UMR frequency", x = unit(0.015, "npc"), y = unit(0.83, "npc"), rot = 90, gp=gpar(fontsize=15))
grid.text("Enrichment", x = unit(0.015, "npc"), y = unit(0.6, "npc"), rot = 90, gp=gpar(fontsize=15))
grid.text("No. of genes", x = unit(0.015, "npc"), y = unit(0.3, "npc"), rot = 90, gp=gpar(fontsize=15))
pdf("Figure3a.pdf", height = 10, width = 10)
grid.newpage()
grid.draw(GW_scale_figure_gt)
grid.text("UMR frequency", x = unit(0.015, "npc"), y = unit(0.83, "npc"), rot = 90, gp=gpar(fontsize=15))
grid.text("Enrichment", x = unit(0.015, "npc"), y = unit(0.6, "npc"), rot = 90, gp=gpar(fontsize=15))
...
```{r GW_enhancer_TFBS, fig.height=5, fig.width=8}
(homer_unique_enhancer_GW_GW17only_figure)
ggsave(homer_unique_enhancer_GW_GW17only_figure, file = "Figure3b.pdf", height = 4, width = 8)
```

### Figure 3c: OLIG2

* UCSC genome browser screen shot of GW17 UMR activated Oligodendrocyte Lineage Transcription Factor 2 (OLIG2).     
...
ggsave(unique_enhancer_GW_Olig2_DE_DAVID_figure, file = "Figure3d.pdf", height = 7, width = 8)
```

### Figure 3e: OLIG2

* Cytoscape network of Oligodendrocyte Lineage Transcription Factor 2 (OLIG2) target genes that are differential expressed.       
...
UMR_enhancer_summary[UMR_enhancer_summary$compare == "NPCs" & UMR_enhancer_summary$UMR == "hypo", "UMR"] <- "GW13/Cortex UMRs"
UMR_enhancer_summary[UMR_enhancer_summary$compare == "NPCs" & UMR_enhancer_summary$UMR == "hyper", "UMR"] <- "GW17/GE UMRs"
UMR_enhancer_summary_tall <- melt(UMR_enhancer_summary %>% filter(compare == "GW"), id = c("Sample", "compare", "UMR")) %>%
  mutate(UMR = factor(UMR, levels = c("GW13/Cortex UMRs", "GW17/GE UMRs")))
(UMR_enhancer_summary_figure <- ggplot(UMR_enhancer_summary_tall, aes(x = Sample, y = value, fill = UMR)) + 
   geom_bar(stat = "identity", position = position_dodge(), width = .8) + 
   facet_grid(variable ~ ., scales = "free") + 
   scale_fill_manual(values = c("red", "blue"), name = "", labels = c("GW13 UMRs", "GW17 UMRs")) + 
   xlab("") + 
   ylab("") + 
   theme_bw() + 
...
ggsave(UMR_enhancer_summary_figure, file = "Figure3b.pdf", height = 10, width = 12)
```
...
(DMR_GW_TF_figure <- DMR_GW_TF_figure + 
   ...
   theme(strip.text = element_text(size = 18), axis.title = element_text(size = 18), axis.text = element_text(size = 15), legend.text = element_text(size = 18)))
ggsave(DMR_GW_TF_figure, file = "Figure3c.pdf", height = 6, width = 12)
```
-->
...
* UMRs frequency (bp/MB) for each chromosome for MeDIP UMRs between MZ twins (A), WGBS UMRs between NPCs cortex and GE (B), and WGBS UMRs between gestational weeks (C).     
...
* Fold enrichment on log2 scale for overlapping with genomic regions in MeDIP UMRs between MZ twins (A), WGBS UMRs between NPCs cortex and GE (B), and WGBS UMRs between gestational weeks (C).     
...
* Fraction of differential expressed genes proximally (TSS +/- 1500bp) associated with MeDIP UMRs between MZ twins (A), WGBS UMRs between NPCs cortex and GE (B), and WGBS UMRs between gestational weeks (C).     
...
* DAVID GO biological processes enriched for DE genes between MZ twins (A), between NPCs cortex and GE (B), and between gestational weeks (C).    
...
* (A). For each UMR between NPCs identified by WGBS we calculated the normalized MeDIP-seq (methylated, top panel) and MRE-seq (unmethylated, bottom panel) signal. From this we show boxplot of methylation asymmetry between MeDIP-seq and MRE-seq signals in cortex and GE cells defined as (signal(cortex)-signal(GE))/(signal(cortex)+signal(GE)). (B). UMR frequency (bp/MB) across all chromosomes for MeDIP UMRs between NPCs cortex and GE. (C). FDR for GREAT GO biological processes terms enriched (region-based binomial and hypergeometric FDR < 0.05) in MeDIP NPCs cortex UMRs (red), and GE UMRs (blue) on -log10 scale. 
...
layOut(list(valid_boxplot, 1, 1:2), 
       list(DMR_freq_NPCs_figure + scale_fill_manual(values = c("blue", "red"), labels = c("GE UMRs", "Cortex UMRs"), name = ""), 1, 3:4), 
...
* DAVID enriched terms (FDR < 0.01) for isoforms between NPCs (A) and gestational weeks (B).   
...
* Location of UMRs along each chromosome for MeDIP UMRs between MZ twins (A), WGBS UMRs between NPCs cortex and GE (B), and WGBS UMRs between gestational weeks (C).     
...
* (A). Unique miRNA detected (>0.1 reads per million mapped) across all samples. Heatmap of differential expressed and highly expressed (RPM > 100) miRNAs between MZ twins (B), NPCs cortex and GE (C), and gestational weeks (D).         
...
miR_DE_NPCs_heatmap <- ggplot(melt(t(scale(t(miR_DE_NPCs_heat), center = T, scale = T))) %>% mutate(miR = factor(X1, levels = rev(rownames(miR_DE_NPCs_heat))), Sample = factor(X2, levels = colnames(miR_DE_NPCs_heat))), aes(x=Sample, y=miR, fill=value)) + 
   geom_tile() + 
   scale_fill_gradient(name = "Z-score") + 
   xlab("") + 
   ylab("") + 
...
* Fraction of UMRs along normalized length of the genome in NPCs cortex and GE UMRs (A), and gestational week UMRs (B).      
