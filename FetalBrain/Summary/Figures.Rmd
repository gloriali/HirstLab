...
```{r GW_DE_heatmap}
clab <- data.frame(sample = colnames(GW_DE_rpkm),
                   y = 1, 
...
clab_figure <- ggplot(clab, aes(sample, y, fill = GW)) + 
   ...
   ylab("") + 
   theme_bw() + 
   theme(axis.text = element_text(size = 0), axis.ticks = element_line(color = "white"), panel.border = element_rect(color = "white"), plot.margin=unit(c(1,3.4,-0.3,1), "cm"), legend.position = "top", panel.grid = element_line(color = "white"))
GW_DE_heatmap <- ggplot(melt(t(scale(t(GW_DE_rpkm), center = T, scale = T))) %>% mutate(miR = factor(X1, levels = rev(rownames(GW_DE_rpkm))), Sample = factor(X2, levels = colnames(GW_DE_rpkm))), aes(x=Sample, y=miR, fill=value)) + 
   ...
   ylab("") + 
   theme_bw() + 
   theme(axis.text.y = element_text(size = 0), axis.ticks.y = element_line(color = "white"), panel.border = element_rect(color = "white"), axis.text.x = element_text(angle = 90), plot.margin=unit(c(-0.3,1,1,1), "cm"), panel.background = element_rect(fill = "white"), panel.grid = element_line(color = "white"))
...
(DAVID_MZ_figure <- ggplot(DAVID_MZ, aes(x = Term, y = FDR, fill = Cell)) + 
   ...
   ylab("-log10(FDR)") + 
   theme_bw())
ggsave(DAVID_MZ_figure, file = "~/快盘/FetalBrain/Figures/FigureS5.pdf", height = 12, width = 12)
```
...
(DAVID_neurospheres_figure <- ggplot(DAVID_neurospheres, aes(x = Term, y = FDR, fill = DE)) + 
   ...
   ylab("-log10(FDR)") + 
   theme_bw())
ggsave(DAVID_neurospheres_figure, file = "~/快盘/FetalBrain/Figures/FigureS9.pdf", height = 12, width = 12)
```
...
ggsave(isoform_neurospheres_DAVID, file = "~/快盘/FetalBrain/Figures/FigureS10.pdf", height = 12, width = 12)
```
...
ggsave(DMR_MZ_TF_figure, file = "~/快盘/FetalBrain/Figures/FigureS11.pdf", height = 5, width = 12)
```
...
pdf("~/快盘/FetalBrain/Figures/FigureS12.pdf", height = 12, width = 12)
grid.draw(gt) 
grid.text("Average DNA methylation", x = unit(0.015, "npc"), y = unit(0.65, "npc"), rot = 90)
grid.text("CpG density", x = unit(0.015, "npc"), y = unit(0.15, "npc"), rot = 90)
dev.off()
...
pdf("~/快盘/FetalBrain/Figures/FigureS13.pdf", height = 7, width = 12)
grid.arrange(MeDIP_figure_HuFNSC01, MeDIP_figure_HuFNSC02, nrow = 1, widths = 2:3)
dev.off()
```
...
ggsave(regulator_5mC_heatmap, file = "~/快盘/FetalBrain/Figures/FigureS14.pdf", height = 12, width = 12)
```
...
```{r miR, fig.height = 12, fig.width = 12}
...
grid.text("b", x = unit(0.515, "npc"), y = unit(0.98, "npc"), gp=gpar(fontsize = 20))
grid.text("c", x = unit(0.015, "npc"), y = unit(0.48, "npc"), gp=gpar(fontsize = 20))
pdf("~/快盘/FetalBrain/Figures/FigureS17.pdf", height = 12, width = 12)
layOut(list(GREAT_GW_Cortex_figure, 1:2, 1:4), 
       list(GREAT_GW_GE_figure, 1:3, 5:9), 
       list(GREAT_GW_shared_figure, 3:4, 1:4))
grid.text("a", x = unit(0.015, "npc"), y = unit(0.98, "npc"), gp=gpar(fontsize = 20))
...
```{r GW_TFBS, fig.height = 5, fig.width = 15}
...
ggsave(DMR_GW_TF_figure, file = "~/快盘/FetalBrain/Figures/FigureS18.pdf", height = 12, width = 12)
```
