...
# lum_myo_isoform_intersect <- intersect(lum084_myo084_isoform_all_gene$id, intersect(lum080_myo080_isoform_all_gene$id, lum035_myo035_isoform_all_gene$id))
# write.table(lum_myo_isoform_intersect, file = "~/快盘/REMC/lum_myo_isoform_common.txt", sep = "\t", quote = F, row.names = F, col.names = F)
...
lum_myo_isoform_common_DAVID <- read.delim("~/快盘/REMC/lum_myo_isoform_common_DAVID.txt", as.is=T)
lum_myo_isoform_common_DAVID <- lum_myo_isoform_common_DAVID[lum_myo_isoform_common_DAVID$Benjamini < 0.01, ]
lum_myo_isoform_common_DAVID$Term <- gsub("GO:[0-9]+~", "", lum_myo_isoform_common_DAVID$Term)
lum_myo_isoform_common_DAVID$Term <- gsub("hsa[0-9]+:", "", lum_myo_isoform_common_DAVID$Term)
lum_myo_isoform_common_DAVID$Category <- gsub("GOTERM_MF_5", "GOMF", lum_myo_isoform_common_DAVID$Category)
lum_myo_isoform_common_DAVID$Category <- gsub("GOTERM_BP_5", "GOBP", lum_myo_isoform_common_DAVID$Category)
lum_myo_isoform_common_DAVID$Term <- factor(lum_myo_isoform_common_DAVID$Term, levels = lum_myo_isoform_common_DAVID[order(lum_myo_isoform_common_DAVID$Category, lum_myo_isoform_common_DAVID$Benjamini, decreasing = T),]$Term)
(Enrich_lum_myo_isoform_common <- ggplot(data = lum_myo_isoform_common_DAVID, aes(Term, -log10(Benjamini))) +
   geom_bar(aes(fill = Category), width = .5) + 
   coord_flip() + 
   ylab("-log10(Benjamini P-value)") + 
   geom_text(aes(label = round(-log10(Benjamini), 2), hjust = 0)) + 
   scale_fill_manual(values = c("GOBP" = "blue", "GOMF" = "purple", "KEGG_PATHWAY" = "darkgreen")) +
   theme(axis.title.y = element_text(size = 0), axis.title.x = element_text(size = 15), axis.text = element_text(size = 15, color = "black"), legend.text = element_text(size = 15), legend.title = element_text(size = 15), panel.background = element_rect(fill = "transparent", color = "black"), plot.background = element_rect(fill = "transparent")))
ggsave(Enrich_lum_myo_isoform_common, file = "~/快盘/REMC/figures/Enrich_lum_myo_isoform_common.pdf", width = 11)
```
