...
plot(c(0, 100), c(0, 0.015), type = "n", xlab = "gene position", ylab = "density")
lines(density(na.omit(lum084_myo084_isoform_all$exon_pos)), col = "red", lty = 1, lwd = 3)
lines(density(na.omit(lum084_stem084_isoform_all$exon_pos)), col = "blue", lty = 1, lwd = 3)
lines(density(na.omit(myo084_stem084_isoform_all$exon_pos)), col = "green", lty = 1, lwd = 3)
legend("bottomright", c("lum084 vs myo084", "lum084 vs stem084", "myo084 vs stem084"), col = c("red", "blue", "green"), lty = 1, lwd = 5)
# dev.off()
...
venn_all <- venn.diagram(isoform_all, filename = NULL, fill = rep("white", 3), cex = 0, alpha = 0, col = c("red", "blue", "green"), margin = 0.06, cat.cex = 1.8, category.names = c("lum_myo", "lum_stem-like", "myo_stem-like"), cat.dist = c(-0.01, -0.01, 0.01))
venn_valid <- venn.diagram(isoform_valid, filename = NULL, fill = rep("white", 3), cex = 0, alpha = 0, col = c("red", "blue", "green"), margin = 0.06, cat.cex = 1.8, category.names = c("lum_myo", "lum_stem-like", "myo_stem-like"), cat.dist = c(-0.01, -0.01, 0.01))
...
isoform_all_GO <- data.frame(Term = c(lum084_myo084_isoform_all_GO$Term, lum084_stem084_isoform_all_GO$Term, myo084_stem084_isoform_all_GO$Term), FDR = c(lum084_myo084_isoform_all_GO$FDR, lum084_stem084_isoform_all_GO$FDR, myo084_stem084_isoform_all_GO$FDR), CellType = c(rep("lum vs myo", nrow(lum084_myo084_isoform_all_GO)), rep("lum vs stem-like", nrow(lum084_stem084_isoform_all_GO)), rep("myo vs stem-like", nrow(myo084_stem084_isoform_all_GO))))
terms <- unique(as.character(isoform_all_GO[isoform_all_GO$FDR <= 0.01, ]$Term)) # terms with FDR <= 0.01 in at least one comparisons
isoform_all_GO <- isoform_all_GO[isoform_all_GO$Term %in% terms, ]
isoform_all_GO$Term <- gsub("GO:[0-9]+~", "", isoform_all_GO$Term)
isoform_all_GO$Term <- factor(isoform_all_GO$Term, levels = unique(isoform_all_GO[order(isoform_all_GO$FDR, decreasing = T),]$Term))
(Enrich_isoform_all_GO <- ggplot(data = isoform_all_GO, aes(Term, -log10(FDR)-2)) +
   geom_bar(aes(fill = CellType), width = .5, position="dodge") + 
   scale_y_continuous(breaks = seq(-2, 12, by = 2), labels = as.character(seq(0, 14, by = 2))) + 
   ylab("-log10(FDR)") + 
   coord_flip() + 
   scale_fill_manual(values = c("lum vs myo" = "red", "lum vs stem-like" = "blue", "myo vs stem-like" = "green")) + 
...
ggsave(Enrich_isoform_all_GO, file = "~/快盘/REMC/figures/Enrich_isoform_all_GO.pdf")
