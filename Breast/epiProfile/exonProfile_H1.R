...
(WGBS_CpG_profile <- ggplot(WGBS_CpG, aes(x = Position, y = value, group = group)) + 
   ...
   ylab("") + 
   scale_color_manual(values = c("CpG density" = "black", "H1-specific" = "orange", "myo-specific" = rgb(50,200,50, maxColorValue = 255), "expressed_in_both" = "purple", "not_expressed" = "blue")) + 
...
# grid.newpage()
pdf("WGBS_CpG_profile_H1_myo.pdf", width = 9)
grid.draw(gt) 
...
t.test(na.omit(H1_WGBS_3p_peak[myo_specific, "max"]), na.omit(H1_WGBS_3p_peak[both, "max"]))$p.value
t.test(na.omit(H1_WGBS_3p_peak[myo_specific, "max"]), na.omit(H1_WGBS_3p_peak[H1_specific, "max"]))$p.value
...
t.test(na.omit(H1_WGBS_5p_peak[myo_specific, "max"]), na.omit(H1_WGBS_5p_peak[both, "max"]))$p.value
t.test(na.omit(H1_WGBS_5p_peak[myo_specific, "max"]), na.omit(H1_WGBS_5p_peak[H1_specific, "max"]))$p.value
...
t.test(na.omit(myo_WGBS_3p_peak[myo_specific, "max"]), na.omit(myo_WGBS_3p_peak[both, "max"]))$p.value
t.test(na.omit(myo_WGBS_3p_peak[myo_specific, "max"]), na.omit(myo_WGBS_3p_peak[H1_specific, "max"]))$p.value
...
t.test(na.omit(myo_WGBS_5p_peak[myo_specific, "max"]), na.omit(myo_WGBS_5p_peak[both, "max"]))$p.value
t.test(na.omit(myo_WGBS_5p_peak[myo_specific, "max"]), na.omit(myo_WGBS_5p_peak[H1_specific, "max"]))$p.value
...
(H3K36me3_exons_profile <- ggplot(H3K36me3_exons_stat, aes(x = Expression, group = utilize)) + 
   ...
   scale_fill_manual(values = c("H1" = "orange", "myo" = rgb(50,200,50, maxColorValue = 255))) + 
...
# grid.newpage()
pdf("H3K36me3_exons_profile_H1_myo.pdf", width = 10, height = 7)
grid.draw(gt) 
...
setwd("~/快盘/REMC/epiProfile/")
library(ggplot2)
# get H1-specific, lum-specific, expressed in both cell types exons  
isoform <- read.delim("lum_84_H1_r1a_isoform.txt", head = T, as.is = T)
isoform$coord <- gsub("<[-]*1_ENSG[0-9]+", "", isoform$V1)
isoform$coord  <- gsub("chr", "", isoform$coord)
isoform$coord  <- gsub(":", "_", isoform$coord)
isoform$coord  <- gsub("-", "_", isoform$coord)
...
H1$coord  <- paste0(H1$V2, "_", H1$coord)
rownames(H1) <- H1$coord
lum084 <- read.delim("~/REMC/epiProfile/A17919.G.exn.A.rpkm", head = F, as.is = T)
...
both <- intersect(H1[H1$V4 > 0.1, "coord"], lum084[lum084$V4 > 0.1, "coord"])
neither <- setdiff(H1$coord, c(H1_specific, lum_specific, both))
...
H1_WGBS_3p <- H1_WGBS_3p/10
H1_WGBS_5p <- H1_WGBS_5p/10
WGBS_3p <- data.frame(Cell_type = rep(c("H1", "lum"), each = 20*4), Expression = rep(c(rep(c("H1-specific", "lum-specific", "expressed_in_both", "not_expressed"), each = 20)), 2), Position = rep(seq(-190, 190, by = 20), times = 8), WGBS = -1)
WGBS_3p$group <- interaction(WGBS_3p$Cell_type, WGBS_3p$Expression)
WGBS_3p[WGBS_3p$group == "H1.H1-specific", "WGBS"] <- colMeans(H1_WGBS_3p[H1_specific,], na.rm = T)
WGBS_3p[WGBS_3p$group == "H1.lum-specific", "WGBS"] <- colMeans(H1_WGBS_3p[lum_specific,], na.rm = T)
WGBS_3p[WGBS_3p$group == "H1.expressed_in_both", "WGBS"] <- colMeans(H1_WGBS_3p[both,], na.rm = T)
WGBS_3p[WGBS_3p$group == "H1.not_expressed", "WGBS"] <- colMeans(H1_WGBS_3p[neither,], na.rm = T)
WGBS_3p[WGBS_3p$group == "lum.H1-specific", "WGBS"] <- colMeans(lum_WGBS_3p[H1_specific,], na.rm = T)
...
WGBS_5p <- data.frame(Cell_type = rep(c("H1", "lum"), each = 20*4), Expression = rep(c(rep(c("H1-specific", "lum-specific", "expressed_in_both", "not_expressed"), each = 20)), 2), Position = rep(seq(-190, 190, by = 20), times = 8), WGBS = -1)
WGBS_5p$group <- interaction(WGBS_5p$Cell_type, WGBS_5p$Expression)
WGBS_5p[WGBS_5p$group == "H1.H1-specific", "WGBS"] <- colMeans(H1_WGBS_5p[H1_specific,], na.rm = T)
WGBS_5p[WGBS_5p$group == "H1.lum-specific", "WGBS"] <- colMeans(H1_WGBS_5p[lum_specific,], na.rm = T)
WGBS_5p[WGBS_5p$group == "H1.expressed_in_both", "WGBS"] <- colMeans(H1_WGBS_5p[both,], na.rm = T)
WGBS_5p[WGBS_5p$group == "H1.not_expressed", "WGBS"] <- colMeans(H1_WGBS_5p[neither,], na.rm = T)
WGBS_5p[WGBS_5p$group == "lum.H1-specific", "WGBS"] <- colMeans(lum_WGBS_5p[H1_specific,], na.rm = T)
...
WGBS_CpG$data <- factor(WGBS_CpG$data, levels = c("WGBS", "CpG"))
WGBS_CpG$End <- factor(WGBS_CpG$End, levels = c("5-prime", "3-prime"))
WGBS_CpG$Expression <- factor(WGBS_CpG$Expression, levels = c("H1-specific", "lum-specific", "expressed_in_both", "not_expressed", "CpG density"))
WGBS_CpG$group <- interaction(WGBS_CpG$Cell_type, WGBS_CpG$Expression)
(WGBS_CpG_profile <- ggplot(WGBS_CpG, aes(x = Position, y = value, group = group)) + 
   geom_line(aes(color = Expression, linetype = Cell_type)) + 
   geom_point(aes(color = Expression, shape = Cell_type)) + 
   facet_grid(data ~ End, scales = "free_y") + 
   ylab("") + 
   scale_color_manual(values = c("CpG density" = "black", "lum-specific" = rgb(200,50,0, maxColorValue = 255), "H1-specific" = "orange", "expressed_in_both" = "purple", "not_expressed" = "blue")) + 
   theme(panel.border = element_rect(linetype = "solid", fill = "transparent"), panel.margin = unit(0.75, "lines"), axis.title = element_text(size = 12), legend.text = element_text(size = 12), legend.title = element_text(size = 12), legend.key = element_rect(fill = "transparent"), panel.background = element_rect(fill = "transparent", color = "black"), plot.background = element_rect(fill = "transparent"), strip.text = element_text(color = "black", size = 12, hjust = 0.5, vjust = 0.5), strip.background = element_rect(color = "black")))
library(grid) 
gt <- ggplot_gtable(ggplot_build(WGBS_CpG_profile)) 
# gt$layout 
gt$heights[[4]] <- unit(3.5, "null") 
# grid.newpage()
pdf("WGBS_CpG_profile_H1_lum.pdf", width = 9)
grid.draw(gt) 
grid.text("Average DNA methylation", x = unit(0.015, "npc"), y = unit(0.65, "npc"), rot = 90)
grid.text("CpG density", x = unit(0.015, "npc"), y = unit(0.15, "npc"), rot = 90)
dev.off()
...
t.test(na.omit(H1_WGBS_3p_peak[both, "max"]), na.omit(H1_WGBS_3p_peak[neither, "max"]))$p.value
t.test(na.omit(H1_WGBS_3p_peak[H1_specific, "max"]), na.omit(H1_WGBS_3p_peak[neither, "max"]))$p.value
t.test(na.omit(H1_WGBS_3p_peak[lum_specific, "max"]), na.omit(H1_WGBS_3p_peak[neither, "max"]))$p.value
t.test(na.omit(H1_WGBS_3p_peak[lum_specific, "max"]), na.omit(H1_WGBS_3p_peak[both, "max"]))$p.value
t.test(na.omit(H1_WGBS_3p_peak[lum_specific, "max"]), na.omit(H1_WGBS_3p_peak[H1_specific, "max"]))$p.value
H1_WGBS_5p_peak <- data.frame(id = rownames(H1_WGBS_5p), max = apply(H1_WGBS_5p, 1, max))
rownames(H1_WGBS_5p_peak) <- rownames(H1_WGBS_5p)
t.test(na.omit(H1_WGBS_5p_peak[both, "max"]), na.omit(H1_WGBS_5p_peak[neither, "max"]))$p.value
t.test(na.omit(H1_WGBS_5p_peak[H1_specific, "max"]), na.omit(H1_WGBS_5p_peak[neither, "max"]))$p.value
t.test(na.omit(H1_WGBS_5p_peak[lum_specific, "max"]), na.omit(H1_WGBS_5p_peak[neither, "max"]))$p.value
t.test(na.omit(H1_WGBS_5p_peak[lum_specific, "max"]), na.omit(H1_WGBS_5p_peak[both, "max"]))$p.value
t.test(na.omit(H1_WGBS_5p_peak[lum_specific, "max"]), na.omit(H1_WGBS_5p_peak[H1_specific, "max"]))$p.value
...
t.test(na.omit(lum_WGBS_3p_peak[H1_specific, "max"]), na.omit(lum_WGBS_3p_peak[neither, "max"]))$p.value
t.test(na.omit(lum_WGBS_3p_peak[lum_specific, "max"]), na.omit(lum_WGBS_3p_peak[neither, "max"]))$p.value
t.test(na.omit(lum_WGBS_3p_peak[lum_specific, "max"]), na.omit(lum_WGBS_3p_peak[both, "max"]))$p.value
t.test(na.omit(lum_WGBS_3p_peak[lum_specific, "max"]), na.omit(lum_WGBS_3p_peak[H1_specific, "max"]))$p.value
...
t.test(na.omit(lum_WGBS_5p_peak[H1_specific, "max"]), na.omit(lum_WGBS_5p_peak[neither, "max"]))$p.value
t.test(na.omit(lum_WGBS_5p_peak[lum_specific, "max"]), na.omit(lum_WGBS_5p_peak[neither, "max"]))$p.value
t.test(na.omit(lum_WGBS_5p_peak[lum_specific, "max"]), na.omit(lum_WGBS_5p_peak[both, "max"]))$p.value
t.test(na.omit(lum_WGBS_5p_peak[lum_specific, "max"]), na.omit(lum_WGBS_5p_peak[H1_specific, "max"]))$p.value

t.test(na.omit(H1_WGBS_3p_peak[H1_specific, "max"]), na.omit(lum_WGBS_3p_peak[H1_specific, "max"]))$p.value
t.test(na.omit(H1_WGBS_5p_peak[H1_specific, "max"]), na.omit(lum_WGBS_5p_peak[H1_specific, "max"]))$p.value
t.test(na.omit(H1_WGBS_3p_peak[lum_specific, "max"]), na.omit(lum_WGBS_3p_peak[lum_specific, "max"]))$p.value
t.test(na.omit(H1_WGBS_5p_peak[lum_specific, "max"]), na.omit(lum_WGBS_5p_peak[lum_specific, "max"]))$p.value

######################################################################################################
H1gene <- read.delim("~/REMC/gene/H1_r1a.G.A.rpkm.pc", head = F, as.is = T)
rownames(H1gene) <- H1gene$V1
lum084gene <- read.delim("~/REMC/gene/A17919.G.A.rpkm.pc", head = F, as.is = T)
rownames(lum084gene) <- lum084gene$V1
geneRPKM <- data.frame(gene = H1gene$V1, RPKM = (H1gene$V3 + lum084gene$V3)/2)
rownames(geneRPKM) <- geneRPKM$gene
exons_geneRPKM <- data.frame(exon = H1$coord)
exons_geneRPKM$gene <- gsub("_[0-9XY]+_[0-9_]+", "", exons_geneRPKM$exon)
exons_geneRPKM$geneRPKM <- geneRPKM[exons_geneRPKM$gene, "RPKM"]
...
H1_specific <- intersect(H1_specific, intersect(H1_expressed, lum_expressed))
lum_specific <- intersect(lum_specific, intersect(H1_expressed, lum_expressed))
both <- setdiff(exons_geneRPKM[exons_geneRPKM$H1exon > 0.1 & exons_geneRPKM$lum084exon > 0.1, "exon"], c(H1_specific, lum_specific))
neither <- setdiff(H1$coord, c(H1_specific, lum_specific, both))
# neither <- intersect(neither, exons_geneRPKM[exons_geneRPKM$lum084gene > 0.1 & exons_geneRPKM$H1gene > 0.1, "exon"])
H1_exon_0 <- exons_geneRPKM[exons_geneRPKM$H1exon == 0 & exons_geneRPKM$H1gene > 0.1, "exon"]
...
# normalize signal
(norm <- sum(lum_H3K36me3_exons$V6)/sum(H1_H3K36me3_exons$V6))
H3K36me3_exons <- data.frame(id = c(H1_H3K36me3_exons$V4, lum_H3K36me3_exons$V4), Cell_type = c(rep("H1", nrow(H1_H3K36me3_exons)), rep("lum", nrow(lum_H3K36me3_exons))), Expression = NA, H3K36me3 = c(H1_H3K36me3_exons$V6 * norm, lum_H3K36me3_exons$V6))
H3K36me3_exons[H3K36me3_exons$id %in% both, "Expression"] <- "expressed_in_both"
# H3K36me3_exons[H3K36me3_exons$id %in% exon_0, "Expression"] <- "not_expressed"
H3K36me3_exons[H3K36me3_exons$id %in% H1_exon_0 & as.character(H3K36me3_exons$Cell_type) == "H1", "Expression"] <- "not_expressed"
...
H3K36me3_exons[H3K36me3_exons$id %in% H1_gene_0 & as.character(H3K36me3_exons$Cell_type) == "H1", "Expression"] <- "not_expressed"
H3K36me3_exons[H3K36me3_exons$id %in% lum_gene_0 & as.character(H3K36me3_exons$Cell_type) == "lum", "Expression"] <- "not_expressed"
H3K36me3_exons <- droplevels(H3K36me3_exons[H3K36me3_exons$Expression != "gene_not_expressed", ])
H3K36me3_exons$Expression <- factor(H3K36me3_exons$Expression, levels = c("expressed_in_both", "H1-specific", "lum-specific", "not_expressed"))
H3K36me3_exons$utilize <- interaction(H3K36me3_exons$Cell_type, H3K36me3_exons$Expression)
H3K36me3_exons <- na.omit(H3K36me3_exons)
summary(H3K36me3_exons$utilize)
library(plyr)
...
(H3K36me3_exons_profile <- ggplot(H3K36me3_exons_stat, aes(x = Expression, group = utilize)) + 
   ...
   xlab("Exon group") + 
   ylab("Average H3K36me3 signal") + 
   scale_fill_manual(values = c("lum" = rgb(200,50,0, maxColorValue = 255), "H1" = "orange")) + 
   theme(axis.title = element_text(size = 20), axis.text.x = element_text(size = 15, color = "black"), legend.text = element_text(size = 20), legend.title = element_text(size = 20), legend.key = element_rect(fill = "transparent"), panel.background = element_rect(fill = "transparent", color = "black"), plot.background = element_rect(fill = "transparent"), strip.text = element_text(color = "black", size = 20, hjust = 0.5, vjust = 0.5), strip.background = element_rect(color = "black")))
t.test(H3K36me3_exons[H3K36me3_exons$utilize == "H1.H1-specific" | H3K36me3_exons$utilize == "lum.lum-specific", "H3K36me3"], H3K36me3_exons[H3K36me3_exons$utilize == "lum.H1-specific" | H3K36me3_exons$utilize == "H1.lum-specific", "H3K36me3"], alternative = "greater")$p.value
t.test(H3K36me3_exons[H3K36me3_exons$utilize == "H1.H1-specific", "H3K36me3"], H3K36me3_exons[H3K36me3_exons$utilize == "lum.H1-specific", "H3K36me3"])$p.value
t.test(H3K36me3_exons[H3K36me3_exons$utilize == "lum.lum-specific", "H3K36me3"], H3K36me3_exons[H3K36me3_exons$utilize == "H1.lum-specific", "H3K36me3"])$p.value
t.test(H3K36me3_exons[H3K36me3_exons$utilize == "H1.expressed_in_both", "H3K36me3"], H3K36me3_exons[H3K36me3_exons$utilize == "lum.expressed_in_both", "H3K36me3"])$p.value
t.test(H3K36me3_exons[H3K36me3_exons$utilize == "H1.not_expressed", "H3K36me3"], H3K36me3_exons[H3K36me3_exons$utilize == "lum.not_expressed", "H3K36me3"])$p.value
# fold enrichment between utilized/un-utilized isoform exons
mean(H3K36me3_exons[H3K36me3_exons$utilize == "H1.H1-specific" | H3K36me3_exons$utilize == "lum.lum-specific", "H3K36me3"])/mean(H3K36me3_exons[H3K36me3_exons$utilize == "lum.H1-specific" | H3K36me3_exons$utilize == "H1.lum-specific", "H3K36me3"])
mean(H3K36me3_exons[H3K36me3_exons$utilize == "H1.H1-specific", "H3K36me3"])/mean(H3K36me3_exons[H3K36me3_exons$utilize == "lum.H1-specific", "H3K36me3"])
mean(H3K36me3_exons[H3K36me3_exons$utilize == "lum.lum-specific", "H3K36me3"])/mean(H3K36me3_exons[H3K36me3_exons$utilize == "H1.lum-specific", "H3K36me3"])
mean(H3K36me3_exons[H3K36me3_exons$utilize == "lum.expressed_in_both", "H3K36me3"])/mean(H3K36me3_exons[H3K36me3_exons$utilize == "H1.expressed_in_both", "H3K36me3"])
mean(H3K36me3_exons[H3K36me3_exons$utilize == "lum.not_expressed", "H3K36me3"])/mean(H3K36me3_exons[H3K36me3_exons$utilize == "H1.not_expressed", "H3K36me3"])
...
gt$heights[[3]] <- unit(2, "null") 
# grid.newpage()
pdf("H3K36me3_exons_profile_H1_lum.pdf", width = 10, height = 7)
grid.draw(gt) 
grid.text("Average H3K36me3 signal", x = unit(0.015, "npc"), y = unit(0.65, "npc"), rot = 90)
grid.text("Exon usage", x = unit(0.015, "npc"), y = unit(0.25, "npc"), rot = 90)
grid.text("lum/H1=0.98", x = unit(0.18, "npc"), y = unit(0.955, "npc"))
grid.text("H1/lum=1.15", x = unit(0.34, "npc"), y = unit(0.6, "npc"))
grid.text("lum/H1=1.71", x = unit(0.51, "npc"), y = unit(0.6, "npc"))
grid.text("lum/H1=1.07", x = unit(0.68, "npc"), y = unit(0.6, "npc"))
dev.off()
