...
coltissue<-data.frame(tissue=c("fibr","lum","myo","stem-like","vHMEC"),
                      col=c("blueviolet","red","green","blue","orange"))
...
pdf("isoform_DE.pdf",width=13,height=8)
par(mar = c(5,5,2,15), xpd=TRUE,cex.axis=1.2,cex.lab=1.5)
barx<-barplot(yy, beside=TRUE,col=c("blue","red"), ylim=c(0,5000),cex.names=1.2,names.arg=summary$tissue, axis.lty=1, xlab="Cell type", ylab="#genes")  
error.bar(barx,yy,ee_up,ee_low)
par(new = T,cex.axis=1.2,cex.lab=1.5)
plot(c(0.2,1.5,2.6,3.9,5.1,6.35),summary$Nexons1,col=colNexons1,axes=F,xlab=NA,ylab=NA,type="p",pch=16,cex=1.2,ylim=c(0,120000),xlim=c(0,7))
error.bar(c(0.2,1.5,2.6,3.9,5.1,6.35),summary$Nexon1,eNexons1_up,eNexons1_low,col=colNexons1,length=0.05)
points(c(0.55,1.85,2.95,4.25,5.45,6.75),summary$Nexons2,col=colNexons2,xlab=NA,ylab=NA,type="p",pch=16,cex=1.2)
error.bar(c(0.55,1.85,2.95,4.25,5.45,6.75),summary$Nexons2,eNexons2_up,eNexons2_low,col=colNexons2,length=0.05)
axis(side = 4,cex=1.2)
mtext(side = 4, line = 3, "#expressed exons",cex=1.5)
legend("topright",c("Isoforms","DE genes","Expressed exons"),inset=c(-0.33,0),col=c("blue","red","black"),lty=c(1,1,NA),lwd=c(8,8,1),pch=c(NA,NA,19),cex=1.1,pt.cex=1.2)
legend("right",as.character(coltissue$tissue),inset=c(-0.33,0),col=as.character(coltissue$col),pch=19,cex=1.2,title="#expressed exons",pt.cex=1.2)
dev.off()
