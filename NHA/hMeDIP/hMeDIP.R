...
dev.off()
NHAR_mut_wt_unique <- as.numeric(system("less ./unique2/NHAR_control_NHA_control.NHAR_control.unique.bed | awk '{s=s+$3-$2}END{print s}'", intern = T))
NHAR_mut_vitc_unique <- as.numeric(system("less ./unique2/NHAR_vitc_NHAR_control.NHAR_control.unique.bed | awk '{s=s+$3-$2}END{print s}'", intern = T))
NHAR_mut <- as.numeric(system("/gsc/software/linux-x86_64-centos5/bedtools/bedtools-2.25.0/bin/intersectBed -a ./unique2/NHAR_control_NHA_control.NHAR_control.unique.bed -b ./unique2/NHAR_vitc_NHAR_control.NHAR_control.unique.bed | awk '{s=s+$3-$2}END{print s}'", intern = T))
NHAR_mut_venn <- draw.pairwise.venn(area1 = NHAR_mut_wt_unique, area2 = NHAR_mut_vitc_unique, cross.area = NHAR_mut, category = c("NHAR mut gain", "NHAR vitc loss"), fill = c("red", "blue"), cex = 1.5, cat.pos = c(-20, 160), cat.dist = 0.05, cat.cex = 1.5)
pdf("./unique2/NHAR_mut_venn.pdf", height = 3, width = 4)
grid.draw(NHAR_mut_venn)
dev.off()
