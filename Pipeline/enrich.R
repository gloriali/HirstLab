enrich <- function(name, dirIn = "./enrich/", dirOut = "./enrich/", fdr = 0.01, category = c("GOBP", "GOMF", "KEGG_PATHWAY", "PANTHER_PATHWAY", "REACTOME_PATHWAY", "INTERPRO"), height = 6, width = 9){
  library(ggplot2)
  ...
  enrich <- na.omit(rbind(data.frame(Category = "GOBP", Term = erminej_BP$V2, FDR = erminej_BP$V10), data.frame(Category = "GOMF", Term = erminej_MF$V2, FDR = erminej_MF$V10), data.frame(Category = DAVID$Category, Term = DAVID$Term, FDR = DAVID$FDR)))
...
# Criteria: 
#   ermineJ (GO) results: <name>_erminej_BP.txt and <name>_erminej_MF.txt
#   DAVID (KEGG, PANTHER, REACTOME, INTERPRO) result: <name>_DAVID.txt
# Parameters: 
...
#   fdr: FDR cutoff for enriched terms
