...
nonCGI_promoter=/home/lli/hg19/hg19v69_nonCGI_promoter.bed # $BEDTOOLS/intersectBed -a /home/lli/hg19/hg19v69_genes_TSS_2000.bed -b /home/lli/hg19/CGI.forProfiles.BED -v > /home/lli/hg19/hg19v69_nonCGI_promoter.bed
...
Output files:
$dirOut/genomic.breakdown.summary (fold enrichment for plotting): Name\tTotal No. of CpGs\tIntergenic\tIntron\tExon\tGene\tnonCGI_promoter\tCGI_promoter\tCGI_genebody\tCGI_intergenic\tCGI_shore\t$region_name
...
$dirOut/DMR.<name>.nonCGI_promoter.bed: chr\tstart\tend\tDMR_ID\tEnsembl
...
all_nonCGI_promoter=1553598 # $BEDTOOLS/intersectBed -a $CG -b $nonCGI_promoter -u | wc -l
...
echo -e "$all_total\t$all_gene\t$all_exon\t$all_nonCGI_promoter\t$all_CGI_promoter\t$all_CGI_genebody\t$all_CGI_intergenic\t$all_CGI_shore\t$all_region" | awk '{print "Total No. of CpGs: "$1"\nExpectedPercentage:\n-Intergenic: "($1-$2)/$1"\n-Intron: "($2-$3)/$1"\n-Exon: "$3/$1"\n-Gene: "$2/$1"\n-nonCGI_promoter: "$4/$1"\n-CGI_promoter: "$5/$1"\n-CGI_genebody: "$6/$1"\n-CGI_intergenic: "$7/$1"\n-CGI shore: "$8/$1"\n-""'$region_name'"": "$9/$1}' 
echo -e "Name\tNCpG\tIntergenic\tIntron\tExon\tGene\tnonCGI_promoter\tCGI_promoter\tCGI_genebody\tCGI_intergenic\tCGI_shore\t$region_name" > $dirOut/genomic.breakdown.summary
for dmr in DMR.*.bed; do
    ...
    $BEDTOOLS/intersectBed -a $dmr.tmp -b $nonCGI_promoter -wa -wb | awk '{print $1"\t"$2"\t"$3"\t"$4"\t"$8}' > $dirOut/DMR.$name.nonCGI_promoter.bed
    ...
    dmr_total=$(less $dirOut/DMR.$name.CpG.bed | wc -l)
    dmr_gene=$($BEDTOOLS/intersectBed -a $dirOut/DMR.$name.CpG.bed -b $gene -u | wc -l)
    dmr_exon=$($BEDTOOLS/intersectBed -a $dirOut/DMR.$name.CpG.bed -b $exon -u | wc -l)
    dmr_nonCGI_promoter=$($BEDTOOLS/intersectBed -a $dirOut/DMR.$name.CpG.bed -b $nonCGI_promoter -u | wc -l)
    dmr_CGI_promoter=$($BEDTOOLS/intersectBed -a $dirOut/DMR.$name.CpG.bed -b $CGI_promoter -u | wc -l)
    dmr_CGI_genebody=$($BEDTOOLS/intersectBed -a $dirOut/DMR.$name.CpG.bed -b $CGI_genebody -u | wc -l)
    dmr_CGI_intergenic=$($BEDTOOLS/intersectBed -a $dirOut/DMR.$name.CpG.bed -b $CGI_intergenic -u | wc -l)
    dmr_CGI_shore=$($BEDTOOLS/intersectBed -a $dirOut/DMR.$name.CpG.bed -b $CGI_shore -u | wc -l)
    if [ -f "$region" ]; then
        ...
        echo -e "$name\t$dmr_total\t$dmr_gene\t$dmr_exon\t$dmr_nonCGI_promoter\t$dmr_CGI_promoter\t$dmr_CGI_genebody\t$dmr_CGI_intergenic\t$dmr_CGI_shore\t$DMR_region\t$all_total\t$all_gene\t$all_exon\t$all_nonCGI_promoter\t$all_CGI_promoter\t$all_CGI_genebody\t$all_CGI_intergenic\t$all_CGI_shore\t$all_region" | awk '{print $1"\t"$2"\t"(($2-$3)/$2)/(($11-$12)/$11)"\t"(($3-$4)/$2)/(($12-$13)/$11)"\t"($4/$2)/($13/$11)"\t"($3/$2)/($12/$11)"\t"($5/$2)/($14/$11)"\t"($6/$2)/($15/$11)"\t"($7/$2)/($16/$11)"\t"($8/$2)/($17/$11)"\t"($9/$2)/($18/$11)"\t"($10/$2)/($19/$11)}' >> $dirOut/genomic.breakdown.summary
    else
        echo -e "$name\t$dmr_total\t$dmr_gene\t$dmr_exon\t$dmr_nonCGI_promoter\t$dmr_CGI_promoter\t$dmr_CGI_genebody\t$dmr_CGI_intergenic\t$dmr_CGI_shore\t$all_total\t$all_gene\t$all_exon\t$all_nonCGI_promoter\t$all_CGI_promoter\t$all_CGI_genebody\t$all_CGI_intergenic\t$all_CGI_shore" | awk '{print $1"\t"$2"\t"(($2-$3)/$2)/(($10-$11)/$10)"\t"(($3-$4)/$2)/(($11-$12)/$10)"\t"($4/$2)/($12/$10)"\t"($3/$2)/($11/$10)"\t"($5/$2)/($13/$10)"\t"($6/$2)/($14/$10)"\t"($7/$2)/($15/$10)"\t"($8/$2)/($16/$10)"\t"($9/$2)/($17/$10)}' >> $dirOut/genomic.breakdown.summary        
    fi
    rm $dmr.tmp
done
