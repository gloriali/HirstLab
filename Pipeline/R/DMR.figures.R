DMR_figures <- function(DMR, sample1, sample2, dirOut = getwd(), width = 8, height = 8, figures = c("length", "count", "adjacentDis", "frequency", "position", "circos"), chrs = c(paste0("chr", as.character(1:22)), "chrX"), colname = c("chr", "start", "end", "ID", "DM", "count", "length"), chrlength = read.csv("/home/lli/hg19/chrlen_hg19.csv", as.is = T, row.names = 1), hist_width = 100){
  ...
  if("count" %in% figures){
    DMR_count_stat <- mutate(summarise(group_by(DMR, chr, DM), lower = quantile(count, 0.25), middle = as.numeric(median(length)), upper = quantile(count, 0.75), min = min(count), max = max(count)), ymin = max(min, lower - 1.5*(upper-lower)), ymax = min(max, upper + 1.5*(upper-lower)))
  ...
  if("adjacentDis" %in% figures){
  	...
  	DMR_dis_stat <- mutate(summarise(group_by(DMR, chr, DM), lower = quantile(dis, 0.25), middle = as.numeric(median(length)), upper = quantile(dis, 0.75), min = min(dis), max = max(dis)), ymin = max(min, lower - 1.5*(upper-lower)), ymax = min(max, upper + 1.5*(upper-lower)))
