...
do
    ...
    less $dirOut/$file.tmp.join | awk '{c[$4]=c[$4]+$2; t[$4]=t[$4]+$3; count[$4]=count[$4]+1} END{for(i in count){if(count[i]>2){print i"\t"c[i]"\t"t[i]"\t"count[i] >> "'$dirOut'""/ERROR.""'$file'"".error"}}; for(i in c){print i"\t"c[i]"\t"t[i]}}' > $dirOut/$file.combine
done

echo "Running methyl_diff; output to "$dirOut/DM.$name.p$pth.d$delta.bed

awk 'NR==FNR {h[$1]=$2"\t"$3; next} {if($1 in h){print $0"\t"h[$1]}}' $dirOut/$file2.combine $dirOut/$file1.combine > $dirOut/$name.join
less $dirOut/$name.join | awk '{print $2"\t"$3"\t"$4"\t"$5}' > $dirOut/$name.input
/home/mbilenky/methyl_diff-methyl_diff/methyl_diff $dirOut/$name.input > $dirOut/$name.output
...
rm $dirOut/$file1.* $dirOut/$file2.* $dirOut/$name.join $dirOut/$name.input $dirOut/$name.join $dirOut/$name.output 
